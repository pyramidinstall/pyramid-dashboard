...
export function useOverviewData(data) {
  return useMemo(() => {
    ...
    const rtiWeighted = rtiValue*0.90;
    ...
    const totalFlightWeighted = rtiWeighted+backlogWeighted+skylineRemaining*0.95;

    return {
      yr1Rev, yr2Rev, dayOfYear2, monthly, concentration, activePMs, activeDealers,
      pipelineFace:Math.round(pipelineFace), pipelineWeighted:Math.round(pipelineWeighted),
      totalFlightFace:Math.round(totalFlightFace), totalFlightWeighted:Math.round(totalFlightWeighted),
      rtiValue:Math.round(rtiValue), backlogFace:Math.round(backlogFace), skylineRemaining,
      totalForwardFace:Math.round(pipelineFace+totalFlightFace),
      totalForwardWeighted:Math.round(pipelineWeighted+totalFlightWeighted),
    };
  }, [data]);
}
...
export function useJobsInFlightData(data) {
  return useMemo(() => {
    ...
    return {
      readyToInvoice, rtiTotal:Math.round(rtiTotal), rtiWeighted:Math.round(rtiTotal*0.9),
...
export function useDealerData(data) {
  return useMemo(() => {
    ...
    postOrders.filter(r=>parseBool(r.is_open_quote)).forEach(r=>{dealerPipe[r.customer]=(dealerPipe[r.customer]||0)+parseNum(r.grand_total);});
    ...
    postOrders.forEach(r=>{
      const key=r.customer+'||'+r.pm;
      if (!firstQuotes[key]||r.created_date<firstQuotes[key].date) firstQuotes[key]={date:r.created_date,dealer:r.customer,pm:r.pm};
    });
...
export function useInetData(data) {
  return useMemo(() => {
    if (!data) return null;
    const { installnet } = data;
    const quarters = ['2025Q2','2025Q3','2025Q4','2026Q1','2026Q2'];
    const yr1=installnet.filter(r=>r.year_bucket==='Year 1');
    const decided=yr1.filter(r=>parseBool(r.decided));
    ...
    installnet.filter(r=>['Year 1','Year 2'].includes(r.year_bucket)).forEach(r=>{
      if (!r.pm) return;
      if (!pmMap[r.pm]) pmMap[r.pm]={pm:r.pm,quotes:[],won:0,decided:0,passed:0,byQ:{}};
      const d=pmMap[r.pm]; d.quotes.push(r);
      if (parseBool(r.decided)){d.decided++; if(parseBool(r.won))d.won++;}
      if (parseBool(r.passed)) d.passed++;
      if (!d.byQ[r.quarter]) d.byQ[r.quarter]={quotes:0,won:0,decided:0,passed:0,value:0};
      d.byQ[r.quarter].quotes++;
      d.byQ[r.quarter].value+=parseNum(r.installation_price);
      if (parseBool(r.decided)){d.byQ[r.quarter].decided++; if(parseBool(r.won))d.byQ[r.quarter].won++;}
    ...
    const pmList=Object.values(pmMap).filter(d=>d.quotes.length>=3).map(d=>{
      ...
      const recentVol=(qVols[3]||0)+(qVols[4]||0), prevVol=(qVols[1]||0)+(qVols[2]||0);
      const freqTrend=recentVol>prevVol*1.1?'up':recentVol<prevVol*0.9?'down':'flat';
      const avgValue=Math.round(d.quotes.reduce((s,r)=>s+parseNum(r.installation_price),0)/d.quotes.length);
      ...
      const revenue=d.quotes.filter(r=>parseBool(r.won)).reduce((s,r)=>s+parseNum(r.installation_price),0);
      ...
      return {pm:d.pm,totalQuotes:d.quotes.length,overallCR,qCRs,qVols,qVals,qNoBid,
        freqTrend,avgValue,daysSince,revenue,crAlert,passed:d.passed};
    }).sort((a,b)=>b.totalQuotes-a.totalQuotes);

    const lossReasons={};
    yr1.filter(r=>parseBool(r.sp_lost)).forEach(r=>{
      const reason=r.loss_reason||'Unknown'; lossReasons[reason]=(lossReasons[reason]||0)+1;
    });

    return {overallCR,revenue:Math.round(revenue),decidedCount:decided.length,
      wonCount:won.length,passedCount:passed.length,pmList,lossReasons,quarters};
  }, [data]);
}
